    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold text-neutral-700 flex items-center gap-1.5">
          <MapPin className="h-4 w-4 text-[#1d4f8a]" /> Manzillar kitobi
        </p>
        <button
          onClick={() => setAdding((o) => !o)}
          className="flex items-center gap-1 text-xs font-semibold text-[#1d4f8a] hover:underline"
        >
      ...
      {addresses.map((addr) => (
        <div
          ...
          className={`flex items-start gap-3 rounded-xl border-2 p-3 cursor-pointer transition ${
            selectedId === addr.id
              ? "border-[#1d4f8a] bg-blue-50"
              : "border-neutral-200 bg-white hover:border-neutral-300"
          }`}
        >
          <MapPin className="h-4 w-4 text-[#1d4f8a] mt-0.5 shrink-0" />
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              <p className="text-sm font-semibold text-neutral-800">{addr.label}</p>
              {addr.is_default && (
                <span className="rounded-full bg-blue-100 px-1.5 py-0.5 text-[10px] font-semibold text-[#1d4f8a]">
                  Asosiy
                </span>
              )}
            </div>
          ...
          <div className="flex gap-1 shrink-0">
            {!addr.is_default && (
              <button
                ...
                className="rounded-full p-1.5 hover:bg-blue-50 text-neutral-400 hover:text-[#1d4f8a]"
                title="Asosiy qilish"
              >
      ...
      {adding && (
        <div className="rounded-xl border-2 border-dashed border-blue-200 bg-blue-50/40 p-4 space-y-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            <select
              ...
              className="h-10 rounded-xl border border-neutral-200 bg-white px-3 text-sm outline-none focus:border-[#1d4f8a]"
            >
          ...
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            <input
              ...
              className="h-4 w-4 rounded accent-[#1d4f8a]"
            />
          ...
          <div className="flex gap-2">
            <Button
              ...
              className="flex-1 h-10 rounded-xl bg-[#1d4f8a] text-white hover:bg-[#164078] text-sm"
            >
